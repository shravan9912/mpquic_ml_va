...
import (
	...
	"github.com/shravan9912/mpquic_ml_va/util"
)
...
func (sch *scheduler) getSplitRatioFromModel(bandwidth1 float32, latency1 float32, packetLossPercent1 float32,
	...
	//Features
	input, _ := tf.NewTensor([1][6][1]float32{{{bandwidth1}, {bandwidth2}, {latency1}, {latency2}, {packetLossPercent1},
		{packetLossPercent2}}})
...
//sHRAVAN_ML SCHEDULER ALGO START
func (sch *scheduler) selectShravanMl(s *session, hasRetransmission bool, hasStreamRetransmission bool, fromPth *path) *path {
	// XXX Avoid using PathID 0 if there is more than 1 path
	if len(s.paths) <= 1 {
		if !hasRetransmission && !s.paths[protocol.InitialPathID].SendingAllowed() {
			return nil
...
//Redundant
func (sch *scheduler) selectPathRedundant(s *session, hasRetransmission bool, hasStreamRetransmission bool, fromPth *path) *path {
	// XXX Avoid using PathID 0 if there is more than 1 path
	if len(s.paths) <= 1 {
		if !hasRetransmission && !s.paths[protocol.InitialPathID].SendingAllowed() {
			return nil
	...
	if len(availablePaths) == 0 {
        ...
        if len(availablePaths) == 2 {
               sch.Redundant = true
               sch.Leftpath = s.paths[availablePaths[1]]
               return s.paths[availablePaths[0]]
        }
...
func (sch *scheduler) selectPath(s *session, hasRetransmission bool, hasStreamRetransmission bool, fromPth *path) *path {
	...
	case constants.SCHEDULER_SHRAVAN_ML:
		return sch.selectShravanMl(s, hasRetransmission, hasStreamRetransmission, fromPth)		
	case constants.SCHEDULER_RANDOM:
		return sch.selectPathRandom(s, hasRetransmission, hasStreamRetransmission, fromPth)
	case constants.SCHEDULER_REDUNDANT:
		return sch.selectPathRedundant(s, hasRetransmission, hasStreamRetransmission, fromPth)
...
func PrintSchedulerInfo(config *Config) {
	...
	schedulerList := []string{constants.SCHEDULER_ROUND_ROBIN, constants.SCHEDULER_LOW_LATENCY,
		constants.SCHEDULER_PEEKABOO, constants.SCHEDULER_SHRAVAN_ML, constants.SCHEDULER_ECF, constants.SCHEDULER_DQNA, constants.SCHEDULER_BLEST,
		constants.SCHEDULER_FIRST_PATH, constants.SCHEDULER_LOW_BANDIT, constants.SCHEDULER_RANDOM,
		constants.SCHEDULER_OPTIMUM_SPLIT,constants.SCHEDULER_REDUNDANT}
	if config.Scheduler == "" {
